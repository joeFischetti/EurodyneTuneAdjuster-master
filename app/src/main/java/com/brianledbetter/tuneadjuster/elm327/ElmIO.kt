...
class ElmIO(private val inputStream : InputStream, private val outputStream: OutputStream) {
    ...
    fun start() {
        ...
        try {
            ...
            waitForOk()
            writeString("ST CSEGT 1") // ST command specific to the chip in the OBDLink
                                                // family of devices.  Enables multiframe transmit.
                                                // which is required for sending large chunks of data.
            waitForOk()
    ...
    fun writeBytesNonBlocking(bytes: ByteArray){
        return writeString(HexUtil.bytesToHexString(bytes))
    }
